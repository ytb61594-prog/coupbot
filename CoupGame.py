...
class CoupGame:
    ...
    def actionToCard(self, action):
        """Maps action number to card index.
        Action 0=Tax, 1=Assassinate, 2=Exchange, 3=Steal, 4=Contessa block, 6=Duke block
        Cards: 0=Duke, 1=Assassin, 2=Ambassador, 3=Captain, 4=Contessa
        """
        if action == 0:  # Tax uses Duke
            return 0
        elif action == 1:  # Assassinate uses Assassin
            return 1
        elif action == 2:  # Exchange uses Ambassador
            return 2
        elif action == 3:  # Steal uses Captain
            return 3
        elif action == 4:  # Contessa block uses Contessa
            return 4
        elif action == 6:  # Duke block uses Duke
            return 0
        else:
            return -1  # Invalid action

    def resolveChallenge(self, challenger, personChallenged, action):
        """Returns True if challenged player has the card (challenger loses), False if they don't (challenged loses)"""
        required_card = self.actionToCard(action)
        if required_card == -1:
            # Invalid action, shouldn't happen
            return False
        
        # Check if challenged player has the required card (only check non-dead cards, ignore -2)
        valid_cards = [c for c in personChallenged.cards if c != -2]
        if required_card in valid_cards:
            # They have the card - they win, challenger loses
            # Replace the card at its position (preserve array structure)
            card_index = personChallenged.cards.index(required_card)
            personChallenged.cards[card_index] = self.deck.draw()  # Replace with new card
            self.deck.add(required_card)  # Return the revealed card to deck (shuffles automatically)
            return True  # Challenged player wins (has the card)
        else:
            # They don't have the card - challenger wins, challenged loses
            return False  # Challenged player loses (doesn't have the card)
